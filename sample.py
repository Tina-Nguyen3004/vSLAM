...
from feature_matching import track_features  # We'll use optical flow for tracking on left images.
...
from pose_estimation import apply_ransac_eight_point  # Use your custom RANSAC eight-point implementation.

###############################################################################
# Monocular Pose Estimation Update using Custom Essential Matrix Estimation
###############################################################################
def update_pose_estimate(good_prev, good_curr, K, current_pose):
    ...
    tracked in consecutive frames using a custom RANSAC eight-point algorithm, and then 
    ...
    # Use your custom RANSAC eight-point algorithm to estimate the essential matrix.
    E, inlier_mask, iterations = apply_ransac_eight_point(good_prev, good_curr, K,
                                                          threshold=5.0, desired_confidence=0.80, max_iterations=1000)
    if E is None:
        print("Custom essential matrix estimation failed. Keeping previous pose.")
        return current_pose

    # Recover the relative pose (rotation R and translation t) using cv2.recoverPose.
    _, R, t, mask_pose = cv2.recoverPose(E, good_curr, good_prev, K)

    # Build the relative 4x4 transformation matrix.
    T_rel = np.eye(4)
    T_rel[:3, :3] = R
    T_rel[:3, 3] = t.flatten()

    # Update the global pose by composing the previous global pose with the relative transformation.
...
def main():
    ...
    # Load calibration and compute intrinsic matrix from the left camera ("P0").
    ...
    for frame_idx in range(1, total_frames):
        ...
        # Update the global pose using our custom essential matrix estimation.
